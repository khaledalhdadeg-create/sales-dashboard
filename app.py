...
# -------------------------------------------------------------
# 1. إدارة حفظ واسترجاع تارجت الفروع (Branch Target Persistence)
# -------------------------------------------------------------
...
# Custom CSS for stc Branding & Responsive Design
st.markdown("""
    <style>
...
# -------------------------------------------------------------
# 2. القائمة الجانبية (Sidebar): اختيار الفرع وإعدادات الأدمن
# -------------------------------------------------------------
st.sidebar.markdown("<h2 style='color: #4F008C;'>🏢 Select Branch</h2>", unsafe_allow_html=True)
branch_list = list(all_targets.keys())
selected_branch = st.sidebar.selectbox("Branch Location:", branch_list, index=0)

current_branch_targets = all_targets.get(selected_branch, DEFAULT_BRANCH_TARGETS["The Avenues"])
...
if admin_pwd == ADMIN_PASSWORD:
    ...
    with st.sidebar.form("admin_branch_target_form"):
        new_ga_voice = st.number_input("Target GA Voice:", value=int(current_branch_targets["target_ga_voice"]))
        new_ga_data = st.number_input("Target GA Data:", value=int(current_branch_targets["target_ga_data"]))
        new_renew_voice = st.number_input("Target Renewal Voice:", value=int(current_branch_targets["target_renew_voice"]))
        new_renew_data = st.number_input("Target Renewal Data:", value=int(current_branch_targets["target_renew_data"]))
        new_zeed = st.number_input("Target Zeed:", value=int(current_branch_targets["target_zeed"]))
    ...
    target_ga_voice = current_branch_targets["target_ga_voice"]
    target_ga_data = current_branch_targets["target_ga_data"]
    target_renew_voice = current_branch_targets["target_renew_voice"]
    target_renew_data = current_branch_targets["target_renew_data"]
    target_zeed = current_branch_targets["target_zeed"]
else:
...
# Tab 1: Daily Target & Run-Rate Tracker
with tab1:
