...
# 4. حساب العمولات وبونص قطع الفئة بناءً على حد الـ 75%
def calculate_payout(prod, ach_pct, actual_units):
    # شرط حد الـ 75% للتحويل من Plan A إلى Flat Bonus
    if ach_pct < 0.75:
        # نظام البونص المباشر فقط على الأعداد المحققة
        flat_rates = {
            'GA Voice': 2.0,
            'GA Data': 1.5,
            'Renew Voice': 1.0,
        ...
        return actual_units * flat_rates.get(prod, 0)
    
    # التأهل لخطة Plan A (شريحة العمولة المستحقة)
    bonus_table = {
        'GA Voice': {0.75: 20, 0.9: 25, 1.0: 30, 1.1: 35, 1.2: 40},
        'GA Data': {0.75: 10, 0.9: 15, 1.0: 20, 1.1: 25, 1.2: 30},
        'Renew Voice': {0.75: 5, 0.9: 10, 1.0: 15, 1.1: 20, 1.2: 25},
        'Renew Data': {0.75: 10, 0.9: 15, 1.0: 20, 1.1: 25, 1.2: 30},
        'Zeed': {0.75: 3, 0.9: 5, 1.0: 10, 1.1: 15, 1.2: 20},
    }
...
total_bonus = sum(calculate_payout(p, weighted_ach[p], actual_sales[p]) for p in raw_ach)
...
with res_col2:
    st.metric("Total Incentive Payout", f"{total_bonus:.1f} KD")
